       ...
       stage('Вхід у Docker') {
            steps {
                script {
                    ...
                    withCredentials([usernamePassword(credentialsId: DOCKER_CREDENTIALS_ID, passwordVariable: 'DOCKER_PASSWOR>
        ...
        stage('Білд Docker зображення') {
            steps {
                script {
                    ...
                    sh 'docker build -t manyok007/site:version${BUILD_NUMBER} .'
                }
            }
        }
          ...
          stage('Тегування Docker зображення') {
            steps {
                script {
                    ...
                    sh 'docker tag manyok007/site:version${BUILD_NUMBER} manyok007/site:latest'
                }
            }
        }
        ...
        stage('Пуш у Docker Hub') {
            steps {
                script {
                    ...
                    sh 'docker push manyok007/site:version${BUILD_NUMBER}'
                    sh 'docker push manyok007/site:latest'
                }
            }
        }
        ...
        stage('Запуск Docker контейнера') {
            steps {
                script {
                    ...
                    sh 'docker run -d -p 8081:80 --name ${CONTAINER_NAME} --health-cmd="curl --fail http://localhost:80 || ex>
